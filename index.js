...
class Rsync {
    ...
    args() {
        ...
        Object.keys(this._options).forEach(key => {
            ...
            if (key.length === 1 && typeof value === 'undefined') {
                ...
                long =
                    Array.isArray(value)
                        ? long.concat(value.map(val => buildOption(key, val, escapeShellArg)))
                        : long.concat(buildOption(key, value, escapeShellArg));
            }
        });
    ...
    execute(opts = {}) {
        ...
        this.output(opts.stdoutHandler || this._outputHandlers.stdout, opts.stderrHandler || this._outputHandlers.stderr);
...
const parseFlags = (...flags) => flags.reduce((acc, flag) => acc.concat(typeof (flag) === 'string'
    ? flag.split('')
    : parseFlags(...flag)), []);
/**
